...
**Capabilities:**
- Conversion framework (10-point scorecard: value prop, hero, social proof, CTA, objections, copy, hierarchy, pricing, SEO, mobile)
- Hero optimization (headline formulas, CTA rules, visual proof, ATF composition)
- Copy audit (7 copy rules, banned words scanner, section title formulas, CTA text audit)
- Social proof & pricing (testimonial scoring, logo bar, metrics, pricing tiers, decoy principle)
- SEO & meta (title/description, Open Graph, Twitter Cards, JSON-LD structured data, robots/sitemap)
- Section-by-section analysis (nav, hero, how it works, before/after, features, testimonials, pricing, FAQ, footer)
- Implementation guide with exact find/replace instructions for every recommended change

**Tools:** Read, Grep, Glob, Bash, Chrome DevTools MCP
**Model:** Sonnet
**Requires:** Chrome DevTools MCP server (for live page inspection)

**Automatic trigger:** Use when "landing page", "conversion", "CRO", "copy audit", or "optimize landing page" is mentioned.

**Core Philosophy:** "Every weak headline is lost revenue. Every missing CTA is a leaked conversion." Produces exact find/replace instructions that a coding agent can execute directly.
...
ln -s /path/to/picky-skills/picky-landingpage ~/.claude/skills/picky-landingpage
```
...
→ Automatically invokes picky-landingpage
...
│
└── picky-orchestrator/
    └── agent.md                 # Meta-agent for coordinated audits
```
...
## Cross-Skill Severity Normalization

When the orchestrator merges findings from multiple agents, use this canonical mapping:

| Normalized | Design | Security | Tester | Performance | Landing Page |
|-----------|---------|----------|--------|-------------|--------------|
| P0 | Critical | Critical (CVSS 9-10) | Critical | Critical (>1s) | Critical |
| P1 | Major | High (CVSS 7-8.9) | High | High (200ms-1s) | High |
| P2 | Minor | Medium (CVSS 4-6.9) | Medium | Medium (50-200ms) | Medium |
| P3 | Suggestion | Low (CVSS 0.1-3.9) | Low | Low (<50ms) | Low |

This enables unified priority matrices in orchestrated audit reports.
...
### Required for picky-tester, picky-performance, and picky-landingpage:
...
### Adding new landing page checks:
- Edit appropriate file in `picky-landingpage/references/`
- Conversion scorecard: `conversion-framework.md`
- Hero section: `hero-optimization.md`
- Copy rules/banned words: `copy-rules.md`
- Social proof/pricing: `social-proof-pricing.md`
- SEO/meta tags: `seo-meta.md`
- Section architecture: `section-patterns.md`
