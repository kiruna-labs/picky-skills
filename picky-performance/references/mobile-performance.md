...
Use native `overflow: auto` with `overscroll-behavior` for scroll containment:

```css
/* Modern approach (no vendor prefixes needed) */
.scrollable-container {
  overflow: auto;
  overscroll-behavior: contain;  /* Prevents scroll chaining */
}

/* For pull-to-refresh prevention on the page level */
html {
  overscroll-behavior-y: none;
}
```

**Note:** `-webkit-overflow-scrolling: touch` is deprecated. Modern iOS Safari (13+) uses momentum scrolling by default on all `overflow: auto` elements. Remove any `-webkit-overflow-scrolling` declarations found in the codebase.

**Scan for deprecated property:**
```bash
grep -rn "webkit-overflow-scrolling" --include="*.css" --include="*.scss" --include="*.tsx" 2>/dev/null
```
