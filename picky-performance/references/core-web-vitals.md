...
LCP can be decomposed into 4 sub-parts for targeted optimization:

| Sub-Part | Description | Optimization |
|----------|-------------|-------------|
| **TTFB** | Time to First Byte — server response time | CDN, edge computing, server optimization |
| **Resource Load Delay** | Time from TTFB to start of LCP resource loading | Preload LCP image, avoid render-blocking resources |
| **Resource Load Time** | Time to download the LCP resource | Optimize image size, use CDN, modern formats (AVIF/WebP) |
| **Element Render Delay** | Time from resource loaded to element rendered | Reduce render-blocking CSS/JS, avoid `display:none` on LCP |

**Measuring LCP Sub-Parts (Chrome DevTools MCP):**
```javascript
// Measure LCP sub-parts via Performance Observer
new PerformanceObserver((list) => {
  const entries = list.getEntries();
  const lastEntry = entries[entries.length - 1];
  ...
  if (lastEntry.url) {
    ...
    if (resourceEntry) {
      console.log('LCP Sub-Parts:', {
        ...
        resourceLoadDelay: Math.round(resourceEntry.startTime - ttfb) + 'ms',
        resourceLoadTime: Math.round(resourceEntry.responseEnd - resourceEntry.startTime) + 'ms',
        elementRenderDelay: Math.round(lastEntry.startTime - resourceEntry.responseEnd) + 'ms',
        totalLCP: Math.round(lastEntry.startTime) + 'ms'
      });
    }
  }
}).observe({ type: 'largest-contentful-paint', buffered: true });
...
1. If TTFB > 800ms → Fix server/CDN first
2. If Resource Load Delay > 10% of LCP → Add `<link rel="preload">` for LCP resource
3. If Resource Load Time > 40% of LCP → Optimize image (size, format, CDN)
4. If Element Render Delay > 10% of LCP → Remove render-blocking CSS/JS
...
INP consists of 3 phases, each requiring different optimization strategies:

| Phase | Description | Target | Optimization |
|-------|-------------|--------|-------------|
| **Input Delay** | Time from interaction to event handler start | < 50ms | Minimize main thread work, break up long tasks |
| **Processing Time** | Event handler execution time | < 100ms | Optimize handler code, use `requestIdleCallback` for non-critical work |
| **Presentation Delay** | Time from handler complete to next frame painted | < 50ms | Minimize DOM mutations, avoid forced layouts |

**Measuring INP Phases (Chrome DevTools MCP):**
```javascript
// Detailed INP measurement using Event Timing API
new PerformanceObserver((list) => {
  for (const entry of list.getEntries()) {
    // Only care about interactions (not observations)
    ...
    console.log('INP Breakdown:', {
      ...
      name: entry.name,
      totalDuration: Math.round(entry.duration) + 'ms',
      inputDelay: Math.round(inputDelay) + 'ms',
      processingTime: Math.round(processingTime) + 'ms',
      presentationDelay: Math.round(presentationDelay) + 'ms',
...
- **High Input Delay** → Long tasks blocking main thread. Use `yield to main thread` pattern, break up work with `scheduler.yield()` or `setTimeout(0)`
- **High Processing Time** → Heavy event handlers. Defer non-visual work with `requestIdleCallback`, batch DOM reads/writes
- **High Presentation Delay** → Complex rendering after handler. Reduce DOM size, avoid layout thrashing, use `content-visibility: auto`
