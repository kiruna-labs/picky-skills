...
**Scan:**
```bash
# CSP nonce-based patterns
grep -rn "nonce-\|strict-dynamic\|Content-Security-Policy" --include="*.ts" --include="*.js" --include="*.html" 2>/dev/null

# Trusted Types implementation
grep -rn "trustedTypes\|TrustedHTML\|TrustedScript\|TrustedScriptURL\|createPolicy" --include="*.ts" --include="*.js" 2>/dev/null

# DOM XSS sinks that Trusted Types protects
grep -rn "innerHTML\|outerHTML\|insertAdjacentHTML\|document\.write\|eval\|setTimeout.*string\|setInterval.*string" --include="*.ts" --include="*.js" --include="*.tsx" 2>/dev/null
```

**Check:**
- [ ] CSP uses `strict-dynamic` with nonce-based approach (preferred over allowlists)
- [ ] Nonces generated server-side per request with cryptographic randomness
- [ ] `require-trusted-types-for 'script'` directive present
- [ ] Trusted Types policy defined for DOM XSS-prone operations
- [ ] Default Trusted Types policy blocks untyped assignments
- [ ] `script-src` avoids `unsafe-inline` (use nonces instead)
- [ ] `unsafe-eval` not present in production CSP

**Remediation:**
```
Content-Security-Policy:
  script-src 'strict-dynamic' 'nonce-{SERVER_GENERATED}';
