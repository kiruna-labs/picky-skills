...
#### GraphQL Advanced Security

**Scan:**
```bash
# Query complexity/depth limiting
grep -rn "depthLimit\|costAnalysis\|queryComplexity\|maxDepth\|complexityLimit" --include="*.ts" --include="*.js" 2>/dev/null

# Introspection control
grep -rn "introspection\|__schema\|__type\|IntrospectionQuery" --include="*.ts" --include="*.js" 2>/dev/null

# Batching configuration
grep -rn "batch\|allowBatchedHttpRequests\|maxBatchSize" --include="*.ts" --include="*.js" 2>/dev/null
```

**Additional Checks:**
- [ ] Query depth limiting enforced (max 7-10 levels)
- [ ] Query complexity/cost analysis configured with per-field costs
- [ ] Introspection DISABLED in production
- [ ] Batching attacks prevented (limit batch size or disable)
- [ ] Field-level authorization (not just type-level)
- [ ] Persisted/allowlisted queries in production (preferred)
- [ ] Alias-based attack prevention (limit aliases per query)
...
grep -rn "WebSocket\|ws\.\|wss://\|socket\.io\|Socket\.IO\|createServer.*ws" --include="*.ts" --include="*.js" 2>/dev/null

# WebSocket upgrade handling
grep -rn "upgrade\|handleUpgrade\|on.*connection\|on.*message" --include="*.ts" --include="*.js" 2>/dev/null
```

**Check:**
- [ ] Origin header validated on WebSocket upgrade request
- [ ] Authentication token validated during upgrade (not just initial connect)
- [ ] Session/token re-validated periodically on long-lived connections
- [ ] Message schema validation on all incoming messages
- [ ] Message size limits enforced
- [ ] Per-connection and per-IP rate limiting on messages
- [ ] Maximum concurrent connections limited per user/IP
- [ ] Binary message handling validated if supported
- [ ] Heartbeat/ping-pong mechanism prevents zombie connections
- [ ] Graceful disconnection on auth token expiry

### Race Conditions & TOCTOU

**Scan:**
```bash
# Financial operations without locks
grep -rn "balance\|credit\|debit\|transfer\|withdraw\|deposit\|payment" --include="*.ts" --include="*.js" 2>/dev/null | grep -v "test\|spec\|mock"

# Concurrent operations without synchronization
grep -rn "Promise\.all\|Promise\.allSettled\|parallel\|concurrent" --include="*.ts" --include="*.js" 2>/dev/null | grep -i "update\|insert\|delete\|transfer"

# Check-then-act patterns
grep -rn "if.*exists.*then\|if.*count.*==\|findOne.*then.*update\|select.*for update" --include="*.ts" --include="*.js" 2>/dev/null
```

**Check:**
- [ ] Financial operations use database-level locking (SELECT FOR UPDATE)
- [ ] Idempotency keys required for payment/transfer operations
- [ ] Optimistic concurrency control with version fields where appropriate
- [ ] Double-submit prevention (unique transaction IDs)
- [ ] Atomic check-and-update operations (no TOCTOU gaps)
- [ ] Inventory/stock operations use atomic decrement
