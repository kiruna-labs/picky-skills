...
**Scan:**
```bash
# WebAuthn/Passkeys implementation detection
grep -rn "navigator\.credentials\|PublicKeyCredential\|@simplewebauthn\|fido2\|webauthn" --include="*.ts" --include="*.js" --include="*.tsx" 2>/dev/null

# WebAuthn server-side verification
grep -rn "verifyRegistrationResponse\|verifyAuthenticationResponse\|attestation\|assertion" --include="*.ts" --include="*.js" 2>/dev/null
```

**Check:**
- [ ] Registration ceremony validates attestation response
- [ ] Challenge generated server-side with cryptographic randomness
- [ ] Challenge has short expiry (< 5 minutes)
- [ ] Origin validation prevents phishing relying parties
- [ ] Relying Party ID matches expected domain
- [ ] User verification (UV) flag enforced when required
- [ ] Credential public key stored securely server-side
- [ ] Authentication counter validated to detect cloned authenticators
- [ ] Resident vs non-resident credential handling appropriate for use case
- [ ] Cross-device authentication (hybrid transport) tested
- [ ] Account recovery path exists when passkey is lost (fallback auth method)
- [ ] Multiple passkeys per account supported for redundancy
