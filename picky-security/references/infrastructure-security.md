...
**Scan:**
```bash
# Unsafe GitHub Actions expression injection
grep -rn '\${{.*github\.event' --include="*.yml" --include="*.yaml" 2>/dev/null

# Workflow run triggers (potential for PR-based attacks)
grep -rn "pull_request_target\|workflow_run" --include="*.yml" --include="*.yaml" 2>/dev/null

# Actions pinned to SHA vs branch
grep -rn "uses:.*@" --include="*.yml" --include="*.yaml" 2>/dev/null | grep -v "@[a-f0-9]\{40\}"

# Overly permissive workflow permissions
grep -rn "permissions:" --include="*.yml" --include="*.yaml" -A 5 2>/dev/null | grep "write-all\|contents: write"
```

**Check:**
- [ ] No `${{ github.event.*.body }}` or similar in `run:` steps (script injection)
- [ ] `pull_request_target` workflows don't checkout untrusted PR code
- [ ] Actions pinned to full SHA, not branch or tag
- [ ] Workflow permissions follow least privilege (not `write-all`)
- [ ] Secrets not exposed to workflows triggered by forks
- [ ] CODEOWNERS file protects workflow files
...
```bash
# SBOM generation
grep -rn "sbom\|cyclonedx\|spdx\|syft\|trivy" --include="*.yml" --include="*.yaml" --include="*.json" 2>/dev/null

# Container image scanning
grep -rn "trivy\|grype\|snyk.*container\|docker.*scan" --include="*.yml" --include="*.yaml" 2>/dev/null

# Dependency confusion indicators (private registry config)
grep -rn "registry\|@scope\|publishConfig" --include="*.npmrc" --include="package.json" 2>/dev/null
...
- [ ] Private package scopes configured to prevent dependency confusion
- [ ] Lock files committed and integrity verified in CI
- [ ] Dependency review action configured for PRs
- [ ] Renovate/Dependabot configured with auto-merge limits
- [ ] Package provenance verified (npm `--provenance` flag)
