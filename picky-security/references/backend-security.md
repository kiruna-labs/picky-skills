...
#### Race Conditions & TOCTOU Deep Dive

**Scan:**
```bash
# Database operations without transactions
grep -rn "\.update(\|\.delete(\|\.insert(" --include="*.ts" --include="*.js" 2>/dev/null | grep -v "transaction\|trx\|\.transacting"

# Optimistic concurrency (version fields)
grep -rn "version\|updatedAt\|etag\|optimistic" --include="*.ts" --include="*.js" 2>/dev/null | grep -i "lock\|concurr\|conflict"

# Idempotency patterns
grep -rn "idempotency\|idempotent\|Idempotency-Key\|x-idempotency" --include="*.ts" --include="*.js" 2>/dev/null
```

**Check:**
- [ ] Multi-step operations wrapped in database transactions
- [ ] `SELECT FOR UPDATE` used for read-modify-write patterns
- [ ] Version columns used for optimistic concurrency control
- [ ] Idempotency keys accepted and enforced for financial APIs
- [ ] Unique constraints prevent duplicate resource creation
- [ ] Atomic increment/decrement for counters and inventory
- [ ] File operations use advisory locks where appropriate
- [ ] Distributed locks (Redis SETNX) used for cross-process coordination
...
if (user.balance >= amount) {
  await db('users').where({ id }).update({ balance: user.balance - amount });
}

// SECURE: Atomic update with check
