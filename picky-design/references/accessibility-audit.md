...
WCAG 2.2 (published October 2023) adds several new success criteria. These MUST be checked in addition to WCAG 2.1 criteria.
...
**Scan:**
```bash
# Sticky/fixed elements that may obscure focus
grep -rn "position:\s*fixed\|position:\s*sticky\|sticky\|fixed" --include="*.css" --include="*.scss" --include="*.tsx" 2>/dev/null

# z-index stacking that may cover focused elements
grep -rn "z-index:\s*[0-9]\|z-\[" --include="*.css" --include="*.tsx" 2>/dev/null
```

**Check:**
- [ ] Sticky headers don't cover focused elements below them
- [ ] Cookie banners / notification bars don't obscure focus
- [ ] Modals/overlays properly trap and manage focus
- [ ] Bottom navigation bars don't cover focused form fields

### 2.4.12 Focus Not Obscured (Enhanced) — AAA
**Requirement:** No part of the focused component is hidden by author-created content.

### 2.4.13 Focus Appearance — AAA
**Requirement:** Focus indicator has sufficient size (at least 2px perimeter) and contrast (3:1).

**Scan:**
```bash
# Custom focus styles
grep -rn "focus.*outline\|focus.*ring\|focus.*border\|:focus-visible" --include="*.css" --include="*.scss" --include="*.tsx" 2>/dev/null

# Focus indicator size
grep -rn "outline-width\|outline.*2px\|ring-2\|ring-\[" --include="*.css" --include="*.tsx" 2>/dev/null
```

### 2.5.7 Dragging Movements — AA
**Requirement:** Functionality that uses dragging can be achieved with a single pointer without dragging.

**Scan:**
```bash
# Drag implementations
grep -rn "onDrag\|draggable\|useDrag\|react-dnd\|react-beautiful-dnd\|@dnd-kit\|sortablejs\|dragstart\|dragend" --include="*.ts" --include="*.tsx" --include="*.js" --include="*.jsx" 2>/dev/null

# Touch gesture libraries
grep -rn "hammer\|gesture\|swipe\|pan\|pinch" --include="*.ts" --include="*.tsx" --include="*.js" 2>/dev/null
```

**Check:**
- [ ] Every drag-and-drop has an alternative (buttons, select, keyboard arrows)
- [ ] Sliders have text input alternative
- [ ] Sortable lists have move up/down buttons
- [ ] Map interactions have non-drag alternatives

### 2.5.8 Target Size (Minimum) — AA
**Requirement:** Targets have a minimum size of 24x24 CSS pixels, with exceptions.

**Scan:**
```bash
# Small interactive elements
grep -rn "width:\s*\(1[0-9]\|2[0-3]\)px\|height:\s*\(1[0-9]\|2[0-3]\)px\|w-[1-5]\b\|h-[1-5]\b" --include="*.css" --include="*.tsx" 2>/dev/null

# Icon-only buttons (often undersized)
grep -rn "<button.*icon\|<IconButton\|btn-icon\|button.*aria-label" --include="*.tsx" --include="*.jsx" 2>/dev/null
```

**Check:**
- [ ] All clickable/tappable targets are at least 24x24px
- [ ] Icon buttons meet minimum size (add padding if needed)
- [ ] Inline links have sufficient target size or spacing
- [ ] Close/dismiss buttons (especially on modals/toasts) meet minimum

### 3.2.6 Consistent Help — A
**Requirement:** If help mechanisms are repeated across pages, they appear in the same relative order.

**Check:**
- [ ] Help/support link appears in same location on every page
- [ ] Contact information is consistently placed
- [ ] Search functionality appears in the same position
- [ ] Chatbot/help widget appears consistently

### 3.3.7 Redundant Entry — A
**Requirement:** Information previously entered by or provided to the user is auto-populated or available for selection.

**Check:**
- [ ] Multi-step forms pre-fill previously entered data
- [ ] Shipping/billing address has "same as" option
- [ ] Return visitors have data pre-populated where appropriate
- [ ] No unnecessary re-entry of information within a session

### 3.3.8 Accessible Authentication (Minimum) — AA
**Requirement:** No cognitive function test (e.g., remembering a password) is required for authentication, unless an alternative is provided.

**Check:**
- [ ] Password managers are not blocked (no `autocomplete="off"` on login)
- [ ] Copy-paste not disabled on password fields
- [ ] Alternative to cognitive tests (passkeys, magic links, SSO)
- [ ] CAPTCHAs have accessible alternatives

**Scan:**
```bash
# Autocomplete blocking
grep -rn 'autocomplete="off"\|autocomplete="new-password"' --include="*.tsx" --include="*.jsx" --include="*.html" 2>/dev/null | grep -i "login\|signin\|password"

# Copy-paste prevention
grep -rn "onpaste\|onPaste.*prevent\|oncopy\|onCopy.*prevent" --include="*.tsx" --include="*.jsx" 2>/dev/null
```

### 3.3.9 Accessible Authentication (Enhanced) — AAA
**Requirement:** No cognitive function test required at all for authentication (no exceptions for object/content recognition).
...
# Run pa11y
npx pa11y http://localhost:PORT

# Check all pages with Lighthouse
npx lighthouse http://localhost:PORT --only-categories=accessibility --output=html

# HTML validation
npx html-validator-cli --url http://localhost:PORT
```
