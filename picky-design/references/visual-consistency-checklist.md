...
grep -rn "prefers-color-scheme\|dark:\|data-theme\|color-scheme\|darkMode\|dark-mode\|theme.*dark" --include="*.css" --include="*.scss" --include="*.tsx" --include="*.ts" --include="*.js" 2>/dev/null

# Theme provider/context
grep -rn "ThemeProvider\|useTheme\|ColorMode\|ThemeContext\|next-themes" --include="*.tsx" --include="*.ts" --include="*.js" 2>/dev/null

# CSS custom properties for theming
grep -rn "var(--\|--color-\|--bg-\|--text-\|--border-" --include="*.css" --include="*.scss" 2>/dev/null | head -30
```

**Check:**
- [ ] `prefers-color-scheme: dark` media query or theme toggle implemented
- [ ] ALL color tokens have dark mode variants (no hardcoded colors in components)
- [ ] Text contrast meets WCAG AA (4.5:1) in dark mode specifically
- [ ] Images/illustrations adapt to dark mode (not jarring white backgrounds)
- [ ] Shadows adapt (lighter/subtle in dark mode, not pure black)
- [ ] Borders visible in both modes (not disappearing in dark)
- [ ] Form inputs have visible borders/backgrounds in dark mode
- [ ] Status colors (red/green/yellow) maintain meaning and contrast in dark mode
- [ ] No pure white (#fff) text on dark — use slightly muted tones
- [ ] No pure black (#000) backgrounds — use dark grays (e.g., #1a1a2e, #121212)
- [ ] Theme transition is smooth (no flash of wrong theme on load)
- [ ] User preference persisted (localStorage or cookie)

**Visual Inspection (Chrome DevTools MCP):**
```bash
# Toggle dark mode via emulation
# Rendering > Emulate CSS media feature prefers-color-scheme: dark
```
- Screenshot in light mode and dark mode side by side
- Verify no components "break" visually in dark mode
...
grep -rn "@container\|container-type\|container-name\|container:" --include="*.css" --include="*.scss" --include="*.tsx" 2>/dev/null

# Container query polyfill
grep -rn "container-query-polyfill\|cqfill" --include="*.ts" --include="*.js" --include="*.json" 2>/dev/null
```

**Check:**
- [ ] Components that need to respond to their container (not viewport) use `@container`
- [ ] `container-type: inline-size` set on parent containers
- [ ] Container queries used for card grids, sidebar layouts, dashboard widgets
- [ ] Fallback styles provided for browsers without support (if needed)
- [ ] Named containers used to avoid ambiguity in nested contexts
...
grep -rn "margin-inline\|margin-block\|padding-inline\|padding-block\|inset-inline\|inset-block\|border-inline\|border-block" --include="*.css" --include="*.scss" 2>/dev/null

# Physical properties that should be logical (for RTL support)
grep -rn "margin-left\|margin-right\|padding-left\|padding-right\|text-align:\s*left\|float:\s*left\|float:\s*right" --include="*.css" --include="*.scss" 2>/dev/null | head -30
```

**Check:**
- [ ] If internationalization (RTL) is supported, logical properties are used
- [ ] `margin-inline-start` used instead of `margin-left` where appropriate
- [ ] `padding-inline` used instead of `padding-left`/`padding-right`
- [ ] `text-align: start` used instead of `text-align: left`
- [ ] `inset-inline-start` used instead of `left` for positioning
- [ ] Consistent usage — don't mix physical and logical properties

**Note:** Logical properties are required for proper RTL support. If the application does not support RTL, physical properties are acceptable but logical properties are still preferred as future-proofing.
