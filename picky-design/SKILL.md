---
name: picky-design
description: Ultra-thorough design system and UI audit. Checks every component for visual consistency, design token usage, accessibility (WCAG 2.2 AA), and responsive design. Produces scored reports with exact file:line references. Trigger with "design audit", "accessibility audit", "UI consistency check", "picky design".
---
...
- **Launch ONE picky-design agent that handles ALL phases.** Do NOT split phases (colors, typography, spacing, accessibility, etc.) into separate sub-agents. The agent runs all phases sequentially.
